...
// ~~ Killstreaks Remastered ~~
// Added killstreak roll over and killstreak mod
// By H3X1C with special thanks to Emosewaj
//
// -- Killstreaks Settings --
// *Add or take away the number of kills require for any streak (Can be a negative number) - (default=0)*
// scr_killstreak_mod = 0				- Mod is disabled, standard amount of kills required for streaks
// scr_killstreak_mod = 5				- 5 more kill are required for any streak (Example: UAV = 8 kills)
// scr_killstreak_mod = -1				- 1 less kill are required for any streak (Example: UAV = 2 kills)
//
// *Allows players to restart their streak once completed in a single life (default=0)
// scr_killstreak_rollover = 0			- Roll over disabled, killstreaks act as normal
// scr_killstreak_rollover = 1			- Roll over enabled, can complete streak set multiple times in one life
//
...
init()
{
	...
	setDvarIfUninitialized( "scr_killstreak_rollover", 0 );

	level.killStreakMod = getDvarInt( "scr_killstreak_mod" );
	level.killStreakRollOver = getDvarInt( "scr_killstreak_rollover" );
...
checkKillstreakReward( streakCount )
{
	...
	foreach ( streakVal, streakName in self.killStreaks )
	{
		...
		if ( isSubStr( streakName, "-rollover" ) && level.killStreakRollOver == 1)
		{
			useStreakName = strTok( streakName, "-" )[0];
		}
		else if (isSubStr( streakName, "-rollover" ))
		{
			continue;
		}
		else
...
clearRideIntro( delay )
{
	...
	// Black screen fixed for nuke MOAB, (Wouldn't clear the vision)
