...
// ~~ Helicopter Remastered ~~
// Attack Helicopter (Cobra) buffed: (Health, accuracy, fire rate and new missiles)
// 	- 50% Chance of attack helicopter model being a little bird (Higher movespeed, lower HP)
// By H3X1C
//
// -- Helicopter Settings --
// *Attack Helicopter fire rockets toggle 	- (default=1)*
// scr_helicopter_cobra_missile = 0		- Helicopter will no longer shoot missiles at enemy players (This will weaken it's effectiveness)
// scr_helicopter_cobra_missile = 1		- Helicopter will now shoot missiles at an enemy if it expels it's magazine without killing it's target
//
// *Defines the duration of helicopters*
// scr_helicopter_cobra_duration 		- Cobra duration in the sky (default=60)
// scr_helicopter_pavelow_duration 		- Pavelow duration in the sky (default=60)
// scr_helicopter_apache_duration 		- Apache duration in the sky (default=40)
//
// Developer Notes
// Cobra = helitype="", Pavelow = helitype="flares", Apache = helitype="minigun"
...
init()
{
	...
	// User configurable dvars
	setDvarIfUninitialized( "scr_helicopter_cobra_missile", 1 );
	setDvarIfUninitialized( "scr_helicopter_cobra_duration", 60 );
	...
	// Helicopter Health Variables
	level.cobra_maxhealth				= 2500;		//Brought more in-line with harrier
	level.pavelow_maxhealth				= 3000;
	level.apache_maxhealth				= 1500;		

	// Helicopter Duration Variables
	level.cobra_duration				= getDvarInt( "scr_helicopter_cobra_duration" );
	level.pavelow_duration				= getDvarInt( "scr_helicopter_pavelow_duration" );
	level.apache_duration				= getDvarInt( "scr_helicopter_apache_duration" );	

	level.cobra_missile_support 		= 1;		// Cobra Missile Support Toggle												
	level.cobra_turretClipSize 			= 30;		// Cobra turret clip size	-	OrigVal: 40
	...
	level.heli_armor_bulletdamage 		= 0.2;		// Damage multiplier to bullets onto helicopter's armor
...
heli_think( lifeId, owner, startnode, heli_team, heliType )
{
	...
	chopper.little_bird = little_bird;
	chopper.lifeId = lifeId;
	chopper.team = heli_team;
	chopper.pers["team"] = heli_team;	
	chopper.owner = owner;

	// DEFINES CHOPPER HEALTH
	if ( heliType == "minigun" )
		chopper.maxhealth = level.pavelow_maxhealth;				// Apache Max health
	...
	if ( heliType == "flares" || heliType == "minigun" )			// Flares for pavelow and apache gunship
...
heli_health()
{
	...
	for ( ;; )
	{
		if ( self.damageTaken >= (self.maxhealth * 0.33) && damageState == 3 )
		{
			...
			playFxOnTag( level.chopper_fx["damage"]["light_smoke"], self, tag );
		}
		else if ( self.damageTaken >= (self.maxhealth * 0.66) && damageState == 2 )
		{
			...
			stopFxOnTag( level.chopper_fx["damage"]["light_smoke"], self, tag );
			playFxOnTag( level.chopper_fx["damage"]["heavy_smoke"], self, tag );
		}
		else if( self.damageTaken > self.maxhealth )
		{
			...
			stopFxOnTag( level.chopper_fx["damage"]["heavy_smoke"], self, tag );
			...
			{
				playFxOnTag( level.chopper_fx["damage"]["on_fire"], self, tag );
...
heli_secondary_explosions()
{
	...
	playFxOnTag( level.chopper_fx["explode"]["large"], self, tag );
...
attack_primary()
{
	...
	while ( 1 )
	{
		...
		// 3/5 chance that it will use the targets angle as a base
		// If not it's totally random								
		if ( randomInt(5) < 3 )
		...
		// Get the x y coords to intersect
		xOffset = cos( angle ) * radiusOffset;		// X
		yOffset = sin( angle ) * radiusOffset;		// Y

		// Target offset, set to our calculated x and y coords based off the intersect position of the angle (40 for player torso)
		...
		for ( i = 0; i < level.cobra_turretClipSize; i++ )
		{
			self fireWeapon( "tag_flash" );

			// Wait time inbetween shots
			if ( i < level.cobra_turretClipSize - 1 )
			{
				wait 0.01;	// Fire rate
			}
			else if ( isAlive( currentTarget ) && level.cobra_missile_support )
			{
				self setVehWeapon( "cobra_FFAR_mp" );
				self fireWeapon( "tag_flash" );
			}

			// Target undefined, Abort
			if ( !isDefined( currentTarget ) )
				break;

			// Check if target still visible, if so increase accuracy 
			if ( self Vehicle_CanTurretTargetPoint( currentTarget.origin+(0,0,40), 1, self ) )
			{
				// Starts at 1, decrements by 0.05 each time till it reaches 0
			...
			// Abort in the event target has gone out of sight.
			if ( shotsSinceLastSighting > 15 ){
				break;
			}

			targetPos = ( (xOffset*convergenceMod)+randomFloatRange( -5, 5 ),(yOffset*convergenceMod)+randomFloatRange( -5, 5 ),40+randomFloatRange( -5, 5 ) );
...
heli_leave()
{
	...
	self heli_reset();
	self stopLoopSound(); // Fix to prevent sound looping while exiting
